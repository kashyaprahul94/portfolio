...
import { Props, SegmentProps, DonutItem, LegendItemProps, Item } from "./types";
  ...
  return (
    <SegmentContainer>
      <circle
        ...
        stroke={color}
        strokeWidth={stroke}
        strokeDasharray={strokeDasharray ? `${strokeDasharray}` : undefined}
        style={{
          strokeDashoffset,
...
const Donut: FunctionComponent<Props> = ({ size, stroke, items, legend }) => {
  ...
  const donutItems = restItems.reduce(
    (acc: ReadonlyArray<DonutItem>, item: Item, index: number) => {
